# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [0.1.5] - 2026-08-05

### Added
- Vertex-list manipulation operations: `reverse`, `append`, `prepend`, `close`, and `join` (see `src/center_of_mass_spiral/operations.py`).
- Unit tests for the new operations (`tests/test_operations.py`).
- Example demo of contour integration on the complex plane

### Changed
- Allow creation of open arc via the Ellipse class
- Enhanced plotting capabilities with `show_detail` parameter in `plot_segment()` and `plot_segments()` functions to visualize segment centroids, tangents, and normals.

## [0.1.4] - 2026-08-01

### Added
- Complex path integration support via `is_complex` parameter in `integrate()` method

### Changed
- Optimized plotting performance with 200x speedup
- Refactored internal data structures:
  - Replaced separate `cx` and `cy` arrays with unified vector `C`
  - Added tangent (`T`) and normal (`N`) vector arrays (2D only)
  - Moved geometric properties (`C`, `T`, `N`) to `Segment` level for better encapsulation

## [0.1.3] - 2026-07-17

### Changed
- Fix issue with ci.yml by adding tags to the on condition


## [0.1.2] - 2026-07-17

### Changed
- GitHub workflow now creates a GitHub Release instead of publishing to PyPI.

## [0.1.1] - 2026-07-17

### Changed
- Auto pick version from pyproject.toml in `__init__.py` using `importlib.metadata.version()`.

## [0.1.0] - 2026-07-17

### Added
- Initial release of the center_of_mass_spiral library
- Core `PolySegment` class for representing polygonal curves
- Generic `integrate()` method with support for:
  - Scalar, array, and callable integrands
  - Cumulative and total integration modes
  - Multi-dimensional integrand arrays
- `get_com_spiral()` function for computing center of mass spirals
- Support for non-uniform density functions
- Built-in shapes: `Ellipse`, `NGon`, `Parabola`, `Flower`
- Geometric context with arc length, centers, tangents, and normals
- Comprehensive test suite with 22+ tests
- Modern Python packaging with `pyproject.toml`
- GitHub Actions CI workflow
- Example applications demonstrating library usage

### Project Structure
- Adopted modern Python project structure with `src/` layout
- Separated library code, examples, and tests into distinct directories
- Added GitHub Actions for continuous integration
- Comprehensive documentation and examples

[0.1.0]: https://github.com/ChenguangZhang/center_of_mass_spiral/releases/tag/v0.1.0
